...
class ChirpstackClient:
    ...
    def __init__(self, server_address, api_token):
        try:
            ...
            print("成功连接到 ChirpStack gRPC 服务。")
        except Exception as e:
            print(f"连接 ChirpStack gRPC 服务失败: {e}")
            self.client = None

    def send_downlink(self, dev_eui, f_port, data_bytes=None, confirmed=False):
        if not self.client:
            print("ChirpStack 客户端未初始化，无法发送下行消息。")
            return None
        ...
        try:
            ...
            print(f"下行消息已入队 -> DEV_EUI: {dev_eui}, FPort: {f_port}, ID: {resp.id}")
            return resp.id
        except grpc.RpcError as e:
            print(f"发送下行到 {dev_eui} 失败: {e.details()}")
            return None
...
class StatusServerClient:
    ...
    def _make_request(self, endpoint, method='GET', params=None, json_data=None):
        ...
        try:
            ...
            print(f"成功同步到状态服务器: {endpoint}, 参数: {params or json_data}")
            return True
        except requests.exceptions.RequestException as e:
            print(f"请求状态服务器时发生错误: {endpoint}, 错误: {e}")
            return False
